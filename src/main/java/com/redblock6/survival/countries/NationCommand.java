    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (command.getName().equalsIgnoreCase("country")) {
            ...
            if (args[0].equalsIgnoreCase("create")) {
                ...
                Countries t = Countries.createTruce(args[1], p);
            }
            if (args[0].equalsIgnoreCase("disband")) {
                ...
                Countries nations = Countries.getTruce(p);
                // Check if player has permission to set truce description
                if (!nations.getRole(p).equals(CountriesRole.Leader)) {
            ...
            if (args[0].equalsIgnoreCase("info")) {
                if (args.length == 1) {
                    ...
                    Countries nations = Countries.getTruce(args[1]);
                    ArrayList<String> msgs = nations.getInfoMessage();
                    for (String s : msgs) {
                        p.sendMessage(s);
                    }
            ...
            if (args[0].equalsIgnoreCase("who")) {
                ...
                if (!Countries.isInTruce(off)) {
                    p.sendMessage(translate("&4&l> &c" + off + " &fis not in a nation"));
                    return true;
                }
                Countries nations = Countries.getTruce(off);
                ArrayList<String> msgs = nations.getInfoMessage();
                for(String msg : msgs) {
                    p.sendMessage(msg);
                }
            ...
            if (args[0].equalsIgnoreCase("desc")) {
                ...
                if (args.length == 2 && args[1].equalsIgnoreCase("reset")) {
                    nations.setDescription(Countries.DEFAULT_DESCRIPTION);
                ...
                nations.setDescription(description);
            ...
            if (args[0].equalsIgnoreCase("invite")) {
                ...
                if (!nations.getRole(p).equals(CountriesRole.Leader)) {
                    p.sendMessage(translate("&4&l> &fYou must be a country &4&lLEADER &fto use this command!"));
                    p.playSound(p.getLocation(), Sound.BLOCK_NOTE_BLOCK_BASS, 100, 1);
                    return true;
                }
                if (nations.getMembers().size() >= Countries.MAX_MEMBERS) {
                ...
                if (Countries.isInTruce(invitee)) {
            ...
            if (args[0].equalsIgnoreCase("promote")) {
                ...
                if (nations.getRole(off).equals(CountriesRole.Leader)) {
                    p.sendMessage(translate("&4&l> &fThis member is already a &4&lLEADER&f!"));
                    p.playSound(p.getLocation(), Sound.BLOCK_NOTE_BLOCK_BASS, 100, 1);
                    return true;
                }
                nations.setRole(off, CountriesRole.Leader);
                for (Player member : nations.getOnlineMembers()) {
                    if (member.equals(p)) {
                        continue;
                    }
                    if (member.equals(off)) {
                        member.sendMessage(translate("&2&l> &fYou have been promoted to &2&lLEADER&f!"));
                        member.playSound(p.getLocation(), Sound.ENTITY_PLAYER_LEVELUP, 100, 1);
                        continue;
                    }
                    member.sendMessage(translate("&2&l> &a" + off.getName() + " &fhas been promoted to &2&lLEADER&f!"));
                    member.playSound(p.getLocation(), Sound.BLOCK_NOTE_BLOCK_PLING, 100, 1);
                }
                p.sendMessage(translate("&2&l> &fSuccessfully promoted &a" + off.getName() + " &fto &2&lLEADER&f!"));
                p.playSound(p.getLocation(), Sound.BLOCK_NOTE_BLOCK_XYLOPHONE, 100, 2);
                p.playSound(p.getLocation(), Sound.BLOCK_NOTE_BLOCK_BASS, 100, 2);
            }
            ...
            if (args[0].equalsIgnoreCase("kick")) {
                ...
                // Check if player has permission to set truce description
                if (!nations.getRole(p).equals(CountriesRole.Leader)) {
                    p.sendMessage(translate("&4&l> &fYou must be a nation &4&lLEADER &fto send that command!"));
                    p.playSound(p.getLocation(), Sound.BLOCK_NOTE_BLOCK_BASS, 100, 1);
                    return true;
                }
                if (args.length < 2) {
                ...
                for (OfflinePlayer op : nations.getLeaders()) {
                    if (member.equals(op)) {
                ...
                for (Player on : nations.getOnlineMembers()) {
                    ...
                    if (nations.getRole(p).equals(CountriesRole.Resident)) {
                        kickerName = "&7&lRESIDENT &f" + p.getName();
                    }
                    if (on.equals(p)) {
            ...
            if (sender instanceof Player) {
                ...
                if (args.length == 1) {
                    ...
                    if (args[0].equalsIgnoreCase("info")) {
                        ...
                        for (Countries nations : Countries.getAllTruces()) {
                    ...
                    if (args[0].equalsIgnoreCase("join")) {
                        ArrayList<String> names = new ArrayList<>();
                        Aplayer ap = Aplayer.getAplayer(p);
                        for(Countries t : ap.getTruceInvites()) {
                            names.add(t.getName());
                        }
                        StringUtil.copyPartialMatches(args[1], names, completions);
                    }
                    if (args[0].equalsIgnoreCase("promote")) {
                        ArrayList<String> names = new ArrayList<>();
                        for(OfflinePlayer off : Countries.getTruce(p).getMembers()) {
